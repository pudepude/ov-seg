...
from segment_anything_hq import sam_hq_model_registry, SamAutomaticMaskGeneratorHQ
...
def infer_sam_type(sam_path, sam_type):
    if sam_type is not None:
        return sam_type
    if "vit_h" in sam_path: return "vit_h"
    if "vit_l" in sam_path: return "vit_l"
...
class SAMVisualizationDemo(object):
    def __init__(self, cfg, granularity, sam_path, ovsegclip_path, instance_mode=ColorMode.IMAGE, parallel=False, use_hq=False, sam_type=None):
        ...
        sam_type = infer_sam_type(sam_path, sam_type)
        if use_hq:
            sam = sam_hq_model_registry[sam_type](checkpoint=sam_path).cuda()
            self.predictor = SamAutomaticMaskGeneratorHQ(sam, points_per_batch=16)
        else:
