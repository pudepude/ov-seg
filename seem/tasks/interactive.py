...
def interactive_infer_image(model, audio_model, image, tasks, refimg=None, reftxt=None, audio_pth=None, video_pth=None):
    ...
    with torch.no_grad():
        model.model.sem_seg_head.predictor.lang_encoder.get_text_embeddings(
            metadata.stuff_classes, is_eval=True)

    all_classes = [name.replace('-other', '').replace('-merged', '') for name in metadata.stuff_classes]
    colors_list = [(np.array(color) / 255).tolist() for color in metadata.stuff_colors]
